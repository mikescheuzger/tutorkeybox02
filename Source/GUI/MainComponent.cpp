...
MainComponent::MainComponent() {
    ...
    // 2. Wire bidirectional MIDI callback to MacroEngine
    audioEngine.onMidiMessageReceived = [this](const juce::MidiMessage& msg) {
    ...
    // 4. Add 4 Layer Cards
    for (int i = 0; i < 4; ++i) {
        layerCards[i] = std::make_unique<LayerCardComponent>(audioEngine, presetManager, i);
