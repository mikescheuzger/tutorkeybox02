...
LayerCardComponent::LayerCardComponent(AudioEngine& engineToControl, PresetManager& presetTarget, int idx)
    : audioEngine(engineToControl), presetManager(presetTarget), layerIndex(idx) {

    titleLabel.setText("LAYER " + juce::String(layerIndex + 1), juce::dontSendNotification);
    titleLabel.setFont(juce::FontOptions(12.0f, juce::Font::bold));
    titleLabel.setColour(juce::Label::textColourId, juce::Colours::white);
    addAndMakeVisible(titleLabel);

    addAndMakeVisible(instrumentBadgeButton);
    instrumentBadgeButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff27272a));
    instrumentBadgeButton.setColour(juce::TextButton::textColourOffId, juce::Colour(0xff38bdf8));
    instrumentBadgeButton.onClick = [this] {
        if (editorWindow == nullptr) {
            editorWindow = std::make_unique<InstrumentEditorWindow>(audioEngine, presetManager, layerIndex);
        }
    ...
    clearButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff3f3f46));
    clearButton.setColour(juce::TextButton::textColourOffId, juce::Colours::lightgrey);
    ...
    resonanceKnob.setValue(presetManager.getLayerPreset(layerIndex).filterResonanceQ);
    ...
    volumeLabel.setFont(9.0f);
    volumeLabel.setJustificationType(juce::Justification::centred);

    addAndMakeVisible(muteToggle);
    muteToggle.setToggleState(presetManager.getLayerPreset(layerIndex).muted, juce::dontSendNotification);
...
void LayerCardComponent::updateFromPreset() {
    ...
    juce::String name = juce::File(layer.sampleContainerPath).getFileNameWithoutExtension();
    instrumentBadgeButton.setButtonText(name.isNotEmpty() ? name.toUpperCase() : "NO INSTRUMENT LOADED");
}

void LayerCardComponent::paint(juce::Graphics& g) {
    auto bounds = getLocalBounds().toFloat().reduced(2.0f);
    g.setColour(isDragHovering ? juce::Colour(0xff0284c7) : juce::Colour(0xff18181b));
    g.fillRoundedRectangle(bounds, 8.0f);

    g.setColour(isDragHovering ? juce::Colour(0xff38bdf8) : juce::Colour(0xff27272a));
...
bool LayerCardComponent::isInterestedInFileDrag(const juce::StringArray& files) {
...
void LayerCardComponent::filesDropped(const juce::StringArray& files, int /*x*/, int /*y*/) {
    ...
    for (const auto& path : files) {
        ...
        if (f.existsAsFile() && f.getFileExtension().equalsIgnoreCase(".bin")) {
            SampleContainerReader::loadContainerFile(f, audioEngine.getSynth(), layerIndex);
            auto layer = presetManager.getLayerPreset(layerIndex);
